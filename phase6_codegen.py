...
Translates optimized Three-Address Code into stack-based pseudo-assembly.
Instructions: LOAD, STORE, ADD, SUB, MUL, DIV, CLT, CGT, CLE, CGE, CEQ, CNE,
              JZ, JMP, PUSH, CALL, HALT
Run standalone: python phase6_codegen.py
"""
...
def code_gen(optimized_ir):
    ...
    # collect user-declared variables (not temporaries like t1, t2…)
    declared = set()
    ...
    def clean(instr):
        """Strip inline optimizer comments."""
...
if __name__ == '__main__':
    print('=' * 50)
    print('  PHASE 6 — Code Generation (Pseudo-Assembly)')
    print('=' * 50)
    _, asm = compile_to_asm(SAMPLE_SOURCE)
    print()
    for line in asm:
        print(f'  {line}')
    print(f'\nTotal assembly lines: {len(asm)}')
